...
class TableDefinition {
    ...
    for (columnName in newSchema.columns) {
      ...
      if (!oldColumnDefinition) {
        ...
        operations.push(Operation.create(
          ...
          'MODIFY COLUMN ' + pool.escapeId(columnName) + ' ' + newColumnDefinition.$toSQL(),
          [columnName]
        ));
      }
    }
  ...
  _schemaToSQL() {
    ...
    if (schema.primaryKey) {
      sql += this._generateKeySQL(KEY_TYPES.PRIMARY, schema.primaryKey) + separator;
    }

    if (schema.uniqueKeys) {
      for (let i = 0; i < schema.uniqueKeys.length; i++) {
        sql += this._generateKeySQL(KEY_TYPES.UNIQUE, schema.uniqueKeys[i]) + separator;
      }
    }

    if (schema.indexes) {
      for (let i = 0; i < schema.indexes.length; i++) {
        sql += this._generateKeySQL(KEY_TYPES.INDEX, schema.indexes[i]) + separator;
      }
    }

    if (schema.spatialIndexes) {
      for (let i = 0; i < schema.spatialIndexes.length; i++) {
        sql += this._generateKeySQL(KEY_TYPES.SPATIAL, schema.spatialIndexes[i]) + separator;
      }
    }
  ...
  _generateKeySQL(type, columns) {
    ...
    } else if (type === KEY_TYPES.SPATIAL) {
...
function createNormalizedSchema(schema) {
  ...
  const keys = {
    ...
    spatialIndexes: cloneDeep(schema.spatialIndexes),
...
function sqlToSchema(sql) {
  ...
  const schema = {
    ...
    uniqueKeys: generateKeysSchema(createDefinitions, KEY_TYPES.UNIQUE),
