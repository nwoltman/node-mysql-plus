...
const diffArrays = require('./utils/diffArrays');
...
class TableDefinition {
  ...
  _getMigrateColumnsOperations(oldSchema) {
    ...
    for (columnName in newSchema.columns) {
      ...
      } else if (!newColumnDefinition.$equals(oldColumnDefinition)) {
        operations.push(Operation.create(
  ...
  _getMigrateUniqueKeysOperations(oldSchema) {
    const operations = [];
    const diff = diffArrays(oldSchema.uniqueKeys || [], this._schema.uniqueKeys || []);
    const removedKeys = diff.removedItems;
    const addedKeys = diff.addedItems;
    var i;
  ...
  _getMigrateIndexesOperations(oldSchema) {
    const operations = [];
    const diff = diffArrays(oldSchema.indexes || [], this._schema.indexes || []);
    const removedKeys = diff.removedItems;
    const addedKeys = diff.addedItems;
    var i;
